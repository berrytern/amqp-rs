...
CERTS_DIR = Path(__file__).resolve().parents[4] / ".certs" / "amqp"

@pytest.mark.asyncio
async def test_tls():
    options = ConfigOptions(queue_name='test_queue', rpc_exchange_name='test_exchange', rpc_queue_name='test_rpc_queue')
    tls_adaptor = TlsAdaptor.with_client_auth(
        str(CERTS_DIR / "ca.pem"),
        str(CERTS_DIR / "rabbitmq_cert.pem"),
        str(CERTS_DIR / "rabbitmq_key.pem"),
        "localhost",
    )
